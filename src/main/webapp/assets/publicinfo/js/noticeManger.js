...
function InitMainTable () {
	$.ajax({
		...
		type : "post",
		dataType : "json",   
		url : V_PATH + "/publicinfo/queryAllNotice.json?r="+Math.random(),
...
function updateContent(id){
	$.ajax({
		...
		dataType : "json",  
		data : {"id": id},
		url : V_PATH + "/publicinfo/queryNoticeById.json?r="+Math.random(),
		success : function(result, textStatus) {
			if(result.state=="T"){
				//成功
				openCalender("修改公告")
...
function deleteRow(id){
	$.ajax({
		...
		dataType : "json",  
		data : {"id": id},
		url : V_PATH + "/publicinfo/deleteNoticeById.json?r="+Math.random(),
		success : function(result, textStatus) {
			if(result.state=="T"){
				//成功重新加载页面
...
function updateInfoSubmit(){
	...
	$.ajax({
		...
		url : V_PATH + "/publicinfo/updateNotice.json?r="+Math.random(),
		success : function(result, textStatus) {
			if(result.state=="T"){
				//成功重新加载页面
				$('#table').bootstrapTable('load', result.data);
...
function createInfoSubmit(){
	...
	$.ajax({
		...
		url : V_PATH + "/publicinfo/addNotice.json?r="+Math.random(),
		success : function(result, textStatus) {
			if(result.state=="T"){
				//成功重新加载页面
				$('#table').bootstrapTable('load', result.data);
...
function searchByName(){
	...
	$.ajax({
		...
		url : V_PATH + "/publicinfo/queryNoticeByAuthor.json?r="+Math.random(),
		success : function(result, textStatus) {
			if(result.state=="T"){
				//成功重新加载页面
				$('#table').bootstrapTable('load', result.data);
